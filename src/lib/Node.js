...
export default class Node {
  ...
  AddToTheta(deltas: VectorRn, index: number) {
    let orgTheta = this.theta;
    this.theta += deltas[index];

    if (CONSTRAIN_THETA) {
      if (this.theta < this.minTheta) this.theta = this.minTheta;
      if (this.theta > this.maxTheta) this.theta = this.maxTheta;
      let actualDelta = this.theta - orgTheta;
      deltas[index] = actualDelta;
    }
